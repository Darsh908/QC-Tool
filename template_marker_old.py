...
Draw boxes around text regions, images, barcodes, and QR codes to create reusable templates.
"""
...
class FieldBox:
    ...
    def __init__(self, name, page, x0, y0, x1, y1, field_type="text", ocr_mode=False):
        ...
        self.x1 = max(x0, x1)
        self.y1 = max(y0, y1)
        self.field_type = field_type  # "text", "image", or "barcode"
...
class TemplateMarkerApp:
    ...
    def _on_release(self, event):
        ...
        if field_name == "Danger Images":
            field_type = "image"
        elif field_name in ["Barcode Number", "QR Code"]:
            field_type = "barcode"
        else:
            field_type = "text"
        ...
        self.status_var.set(f"Added field: {field_name} ({field_type}) on page {self.current_page + 1}")
    ...
    def _edit_selected(self):
        ...
        if new_info:
            ...
            if new_info["name"] == "Danger Images":
                self.selected_field.field_type = "image"
            elif new_info["name"] in ["Barcode Number", "QR Code"]:
                self.selected_field.field_type = "barcode"
            else:
                self.selected_field.field_type = "text"
    ...
    def _toggle_type(self, field):
        # Cycle through types: text -> image -> barcode -> text
